...
// These metrics provide observability into authentication operations, API usage,
// and system health, enabling monitoring, alerting, and troubleshooting.
type Metrics struct {
...
// NewMetrics initializes and returns a new Metrics instance with all collectors registered.
// This function uses promauto to automatically register metrics with the default Prometheus registry.
//
...
// statusCodeToString converts an HTTP status code to a string for use as a Prometheus label.
// This groups status codes into ranges (2xx, 3xx, 4xx, 5xx) while preserving common codes.

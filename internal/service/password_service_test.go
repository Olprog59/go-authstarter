...
func TestPasswordService_RequestPasswordReset(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			svc := NewPasswordService(mockRepo, mockRefreshStore, conf)
			err := svc.RequestPasswordReset(tt.email)
	...
	tests := []struct {
	...
	}{
		{
			...
			setupMock: func(m *mocks.MockUserRepository) {
				...
				m.SetPasswordResetToken("test@example.com", "valid-token", future)
			},
			expectError: false,
		},
		{
			...
			setupMock: func(m *mocks.MockUserRepository) {
				...
				m.SetPasswordResetToken("test@example.com", "expired-token", past)
			},
			expectError:   true,
...
func TestPasswordService_ChangePassword(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			svc := NewPasswordService(mockRepo, mockRefreshStore, conf)
			err := svc.ChangePassword(tt.userID, tt.currentPassword, tt.newPassword)
